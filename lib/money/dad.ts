...
// Kid-side form prompt banks live in lib/money/dad-prompts.ts — they're
// imported by the AskDadPanel CLIENT component, so they had to be split off
// from this file (which uses prisma and is therefore server-only).
//
// Re-export here for back-compat in case anything server-side wants them.
export {
  DAD_GREETINGS,
  DAD_PROMPT_LABELS,
  DAD_AMOUNT_LABELS,
