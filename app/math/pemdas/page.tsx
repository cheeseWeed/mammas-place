...
// Math → PEMDAS / Order of Operations drill (Phase 5).
//
// One expression at a time. Kid types an integer answer. No timer — PEMDAS
// needs thinking time.
//
// Three views in one route (mirrors Phase 2 / 3 / 4):
//   1) Config:   difficulty + round size
//   2) Playing:  one expression at a time, type-the-answer, Enter to submit
...
import {
  DEFAULT_PEMDAS_CONFIG,
  PEMDAS_ROUND_SIZES,
  generatePemdasRound,
  pemdasDifficultyLabel,
  type PemdasConfig,
  type PemdasDifficulty,
  type PemdasProblem,
} from '@/lib/math/pemdas';
import {
  isPending,
  newIdempotencyKey,
  submitEarn,
...
type View =
  | { kind: 'config' }
  | { kind: 'playing'; items: PemdasProblem[]; config: PemdasConfig }
...
function PemdasAuthed() {
  const [view, setView] = useState<View>({ kind: 'config' });

  const startRound = useCallback((config: PemdasConfig) => {
    const items = generatePemdasRound(config.difficulty, config.questions);
...
const DIFFICULTIES: { key: PemdasDifficulty; label: string; blurb: string }[] = [
  { key: 'easy',   label: 'Easy',   blurb: '2 ops, one trap' },
  { key: 'medium', label: 'Medium', blurb: '3 ops or parens' },
  { key: 'hard',   label: 'Hard',   blurb: 'parens + 3-4 ops' },
];

function ConfigCard({ onStart }: { onStart: (c: PemdasConfig) => void }) {
  const [difficulty, setDifficulty] = useState<PemdasDifficulty>(
    DEFAULT_PEMDAS_CONFIG.difficulty,
  );
  const [questions, setQuestions] = useState<number>(DEFAULT_PEMDAS_CONFIG.questions);

  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-sky-200 p-6 md:p-8">
      <div className="text-center mb-6">
        <div className="text-5xl mb-2">🧠</div>
        <h2 className="text-2xl md:text-3xl font-black text-sky-900">Order of Operations</h2>
        <p className="text-sm text-sky-700 mt-1">
          PEMDAS! When +, −, ×, ÷ get tangled, who goes first?
        </p>
      </div>
      ...
      <p className="text-xs text-sky-700 text-center mt-3">
        No timer — take your time. Remember: parentheses first, then × ÷, then + −.
      </p>
    </div>
  );
}
...
function PlayCard({
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-sky-200 p-6 md:p-8">
      ...
      {/* The expression */}
      <div className="text-center mb-6">
        <div className="text-3xl md:text-5xl font-black text-sky-900 tabular-nums">
          {current.prompt} <span className="text-sky-500">= ?</span>
        </div>
      </div>
      ...
      <div className="text-center text-xs text-sky-700 mt-4">
        Order of Operations · {pemdasDifficultyLabel(config.difficulty)}
      </div>
    </div>
  );
}
...
function ResultsCard({
  ...
  const idempotencyKey = useMemo(() => newIdempotencyKey('math-pemdas'), []);
