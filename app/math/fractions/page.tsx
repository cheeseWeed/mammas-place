...
// Math → Fractions & Decimals drill (Phase 4).
//
// One problem at a time. Kid types a number (or a fraction like "3/8");
// answers are parsed to a decimal and compared with tolerance. No timer —
// fractions need thinking time.
//
// Three views: config → playing → results. Mirrors Phase 2 / Phase 3.
...
import {
  DEFAULT_FRACTION_CONFIG,
  FRACTION_ROUND_SIZES,
  formatFractionAnswer,
  fractionDifficultyLabel,
  generateFractionRound,
  isCorrectAnswer,
  type FractionConfig,
...
type View =
  | { kind: 'config' }
  | { kind: 'playing'; items: FractionProblem[]; config: FractionConfig }
...
function FractionsAuthed() {
  const [view, setView] = useState<View>({ kind: 'config' });

  const startRound = useCallback((config: FractionConfig) => {
    const items = generateFractionRound(config.difficulty, config.questions);
...
const DIFFICULTIES: { key: FractionDifficulty; label: string; blurb: string }[] = [
  { key: 'easy',   label: 'Easy',   blurb: 'halves & quarters' },
  { key: 'medium', label: 'Medium', blurb: 'same-denom + decimals' },
  { key: 'hard',   label: 'Hard',   blurb: 'multiply & equivalents' },
];

function ConfigCard({ onStart }: { onStart: (c: FractionConfig) => void }) {
  const [difficulty, setDifficulty] = useState<FractionDifficulty>(
    DEFAULT_FRACTION_CONFIG.difficulty,
  );
  const [questions, setQuestions] = useState<number>(DEFAULT_FRACTION_CONFIG.questions);

  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-sky-200 p-6 md:p-8">
      <div className="text-center mb-6">
        <div className="text-5xl mb-2">🍕</div>
        <h2 className="text-2xl md:text-3xl font-black text-sky-900">Fractions &amp; Decimals</h2>
        <p className="text-sm text-sky-700 mt-1">
          Type a decimal (0.75) or a fraction (3/4) — either works.
        </p>
      </div>
      ...
      <button
        type="button"
        onClick={() => onStart({ difficulty, questions })}
    ...
    <div className="bg-white rounded-3xl shadow-xl border-2 border-sky-200 p-6 md:p-8">
      ...
      <div className="text-center mb-6">
        <div className="text-4xl md:text-5xl font-black text-sky-900 tabular-nums">
          {current.prompt}
          {!current.prompt.includes('=') && <span className="text-sky-500"> = ?</span>}
        </div>
      </div>
      ...
      {reveal !== null ? (
          ...
          {reveal.correct ? (
            ...
            <>
              ...
              <div className="text-4xl font-black text-amber-900 tabular-nums">
                {formatFractionAnswer(reveal.item)}
              </div>
            </>
          )}
        </div>
        ...
        <div>
          <input
            ...
            inputMode="decimal"
            value={typed}
            onChange={(e) => setTyped(e.target.value.replace(/[^\d./-]/g, '').slice(0, 10))}
            onKeyDown={onKeyDown}
            placeholder="Your answer (e.g. 0.75 or 3/4)"
            className="w-full text-center text-2xl md:text-3xl font-black tabular-nums rounded-2xl border-2 border-sky-300 focus:border-sky-500 focus:outline-none bg-sky-50 text-sky-900 px-4 py-4 placeholder:text-sky-400 placeholder:font-medium placeholder:text-base"
      ...
      <div className="text-center text-xs text-sky-700 mt-4">
        Fractions &amp; Decimals · {fractionDifficultyLabel(config.difficulty)}
      </div>
    </div>
  );
}
...
function ResultsCard({
  ...
  const idempotencyKey = useMemo(() => newIdempotencyKey('math-fr'), []);
