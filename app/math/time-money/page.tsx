...
// Math → Time & Money drill (Phase 6).
//
// Real-world math: reading a clock + counting money. Both skills live behind
// the same route — kid picks "Time", "Money", or "Mix" plus a difficulty.
// Each problem carries its own input mode: typed (with normalizer) or
// multiple-choice (pick a button).
//
// Three views: config → playing → results. Mirrors Phase 4 / 5.
...
import {
  DEFAULT_TIME_MONEY_CONFIG,
  TIME_MONEY_ROUND_SIZES,
  generateTimeMoneyRound,
  isCorrectAnswer,
  timeMoneyDifficultyLabel,
  timeMoneySkillLabel,
  type TimeMoneyConfig,
  type TimeMoneyDifficulty,
  type TimeMoneyProblem,
  type TimeMoneySkill,
} from '@/lib/math/time-money';
import {
  isPending,
  newIdempotencyKey,
  submitEarn,
...
export default function TimeMoneyPage() {
  return (
    <LoginGate
      section="math"
      loadingFallback={
...
type View =
  | { kind: 'config' }
  | { kind: 'playing'; items: TimeMoneyProblem[]; config: TimeMoneyConfig }
...
function TimeMoneyAuthed() {
  const [view, setView] = useState<View>({ kind: 'config' });

  const startRound = useCallback((config: TimeMoneyConfig) => {
    const items = generateTimeMoneyRound(config);
...
const SKILLS: { key: TimeMoneySkill | 'mix'; label: string; emoji: string }[] = [
  { key: 'time',  label: 'Time',  emoji: '⏰' },
  { key: 'money', label: 'Money', emoji: '💵' },
  { key: 'mix',   label: 'Mix',   emoji: '🎲' },
];

const DIFFICULTIES: { key: TimeMoneyDifficulty; label: string; blurb: string }[] = [
  { key: 'easy',   label: 'Easy',   blurb: 'hours · coin counts' },
  { key: 'medium', label: 'Medium', blurb: 'quarters · coin sets' },
  { key: 'hard',   label: 'Hard',   blurb: 'elapsed · make change' },
];

function ConfigCard({ onStart }: { onStart: (c: TimeMoneyConfig) => void }) {
  const [skill, setSkill] = useState<TimeMoneySkill | 'mix'>(
    DEFAULT_TIME_MONEY_CONFIG.skill,
  );
  const [difficulty, setDifficulty] = useState<TimeMoneyDifficulty>(
    DEFAULT_TIME_MONEY_CONFIG.difficulty,
  );
  const [questions, setQuestions] = useState<number>(DEFAULT_TIME_MONEY_CONFIG.questions);

  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-sky-200 p-6 md:p-8">
      <div className="text-center mb-6">
        <div className="text-5xl mb-2">⏰💵</div>
        <h2 className="text-2xl md:text-3xl font-black text-sky-900">Time &amp; Money</h2>
        <p className="text-sm text-sky-700 mt-1">
          Read the clock, count the change. Real-world math.
        </p>
      </div>

      <fieldset className="mb-6">
        <legend className="block text-sm font-bold text-sky-900 mb-2">Skill</legend>
      ...
      <button
        type="button"
        onClick={() => onStart({ skill, difficulty, questions })}
      ...
      <p className="text-xs text-sky-700 text-center mt-3">
        No timer. Money: type &quot;$1.25&quot; or &quot;125 cents&quot;. Time: type &quot;3:30&quot;.
      </p>
    </div>
  );
}
...
function PlayCard({
  ...
  useEffect(() => {
    if (reveal === null) {
      setTyped('');
      if (current.mode === 'typed') {
        const t = setTimeout(() => inputRef.current?.focus(), 0);
        return () => clearTimeout(t);
      }
    }
  }, [index, reveal, current.mode]);
  ...
  function submitTyped() {
    ...
    if (trimmed === '') return;
    const isCorrect = isCorrectAnswer(trimmed, current);
    const entry: Answered = { item: current, typed: trimmed, correct: isCorrect };
    setReveal(entry);
    setTimeout(() => advance(entry), isCorrect ? 600 : REVEAL_MS);
  }

  function submitChoice(choice: string) {
    if (reveal !== null) return;
    const isCorrect = isCorrectAnswer(choice, current);
    const entry: Answered = { item: current, typed: choice, correct: isCorrect };
    setReveal(entry);
    setTimeout(() => advance(entry), isCorrect ? 600 : REVEAL_MS);
  }
  ...
  const skillBadge = current.skill === 'time' ? '⏰ Time' : '💵 Money';
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-sky-200 p-6 md:p-8">
      ...
      {/* The prompt */}
      <div className="text-center mb-6">
        <div className="text-xl md:text-2xl font-black text-sky-900 leading-relaxed">
          {current.prompt}
        </div>
      </div>
      ...
      {reveal !== null ? (
        ...
        >
          {reveal.correct ? (
            ...
            <>
              ...
              <div className="text-3xl md:text-4xl font-black text-amber-900 tabular-nums">
                {reveal.item.answer}
              </div>
            </>
          )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {(current.choices ?? []).map((c) => (
            <button
              ...
              className="text-left bg-sky-50 hover:bg-sky-100 border-2 border-sky-300 hover:border-sky-500 text-sky-900 font-bold text-base md:text-lg px-4 py-4 rounded-2xl transition-colors tabular-nums"
            >
              {c}
            </button>
          ))}
        ...
        <div>
          <input
            ref={inputRef}
            type="text"
            inputMode={current.skill === 'time' ? 'numeric' : 'decimal'}
            value={typed}
            onChange={(e) => setTyped(e.target.value.slice(0, 16))}
            onKeyDown={onKeyDown}
            placeholder={current.skill === 'time' ? '3:30' : '$1.25'}
            className="w-full text-center text-3xl md:text-4xl font-black tabular-nums rounded-2xl border-2 border-sky-300 focus:border-sky-500 focus:outline-none bg-sky-50 text-sky-900 px-4 py-4 placeholder:text-sky-400 placeholder:font-medium"
      ...
      <div className="text-center text-xs text-sky-700 mt-4">
        Time &amp; Money · {timeMoneySkillLabel(config.skill)} · {timeMoneyDifficultyLabel(config.difficulty)}
      </div>
    </div>
  );
}
...
function ResultsCard({
  ...
  const idempotencyKey = useMemo(() => newIdempotencyKey('math-tm'), []);
    ...
    (async () => {
      const res = await submitEarn(
        ...
        'round',
        {
          ...
          // No timer — flat zeros so the server's logged meta is honest.
