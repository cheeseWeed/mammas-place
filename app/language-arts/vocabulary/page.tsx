...
// Language Arts → Vocabulary Builder drill (Phase L7).
//
// Same shell as the homophones / thesaurus drills: three views (config /
// playing / results), same MP wiring (submitEarn 'languageArts' 'drill',
// idempotency-keyed).
//
// Three modes mixed within one drill (kid can filter to a single mode or
// leave on 'mixed'):
//   - word-to-def: "What does ARID mean?" → pick a definition
//   - def-to-word: "Which word means 'extremely dry'?" → pick a word
//   - fill-blank:  the word appears (highlighted) in a sentence; kid picks
//                  the meaning that fits the sentence
//
// For fill-blank items the context sentence renders above the prompt with
// the target word bolded — same look as the homophones blank highlight.
...
import {
  ...
  pickRoundItems,
  shuffledChoices,
  type VocabularyDrillItem,
  type VocabularyMode,
  type VocabularyTier,
} from '@/lib/languageArts/vocabulary';
import {
  isPending,
  newIdempotencyKey,
  submitEarn,
...
type ModeOption = VocabularyMode | 'mixed';
...
const DEFAULT_CONFIG: Config = { tier: 'easy', mode: 'mixed', questions: 10 };
const ROUND_SIZES = [5, 10, 15, 20] as const;

type View =
  | { kind: 'config' }
  | { kind: 'playing'; items: VocabularyDrillItem[]; config: Config }
...
function VocabularyAuthed() {
  const [view, setView] = useState<View>({ kind: 'config' });

  const startRound = useCallback((config: Config) => {
    const items = pickRoundItems(
      { tier: config.tier, mode: config.mode },
      config.questions,
    );
    if (items.length === 0) {
      // Some (tier × mode) combos may have zero items as the dataset grows.
...
const TIERS: { key: VocabularyTier; label: string; blurb: string }[] = [
  { key: 'easy', label: 'Easy', blurb: '1st-2nd grade' },
  { key: 'medium', label: 'Medium', blurb: '3rd-4th grade' },
  { key: 'hard', label: 'Hard', blurb: '5th grade+' },
];

const MODE_OPTIONS: { key: ModeOption; label: string; emoji: string }[] = [
  { key: 'mixed', label: 'Mixed', emoji: '🎲' },
  { key: 'word-to-def', label: MODE_META['word-to-def'].label, emoji: MODE_META['word-to-def'].emoji },
  { key: 'def-to-word', label: MODE_META['def-to-word'].label, emoji: MODE_META['def-to-word'].emoji },
  { key: 'fill-blank', label: MODE_META['fill-blank'].label, emoji: MODE_META['fill-blank'].emoji },
];

function ConfigCard({ onStart }: { onStart: (c: Config) => void }) {
  const [tier, setTier] = useState<VocabularyTier>(DEFAULT_CONFIG.tier);
  const [mode, setMode] = useState<ModeOption>(DEFAULT_CONFIG.mode);
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-rose-200 p-6 md:p-8">
      <div className="text-center mb-6">
        ...
        <h2 className="text-2xl md:text-3xl font-black text-rose-900">Vocabulary Builder</h2>
        <p className="text-sm text-rose-700 mt-1">
          Match words to meanings — or pick the right meaning from the sentence.
        </p>
      </div>

      <fieldset className="mb-5">
        <legend className="block text-sm font-bold text-rose-900 mb-2">Mode</legend>
      ...
      <button
        type="button"
        onClick={() => onStart({ tier, mode, questions })}
...
function PlayCard({
  ...
  // Shuffle choices per item so the right answer isn't always in the same
  // slot. Memo on current.id so we don't reshuffle while the reveal animation
  // runs.
  ...
  // def-to-word items have short choices (single words) — use a 2-up grid;
  // word-to-def and fill-blank usually have longer-phrase choices — also 2-up,
  // but render in a single column on narrow widths.
  const longChoices = current.choices.some((c) => c.length > 18);
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-rose-200 p-6 md:p-8">
      ...
      <div
        className={`grid gap-3 ${longChoices ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2'}`}
      >
        {choices.map((choice) => {
          const isPickedRight =
          ...
          return (
            <button
              ...
              className={`rounded-2xl border-2 px-4 py-4 font-bold text-base md:text-lg transition-all text-left ${cls} disabled:cursor-not-allowed`}
            >
              <span>{choice}</span>
      ...
      {reveal !== null && !reveal.correct && (
        <div className="mt-5 rounded-2xl bg-amber-50 border-2 border-amber-200 p-4">
          ...
          <div className="text-sm text-amber-900">
            {current.rule ?? `The right answer here is "${current.answer}".`}
          </div>
        </div>
      )}
    </div>
...
// Render the fill-blank context with **bold** word highlighted. Splits the
// sentence on `**...**` markers and wraps the captured word in a styled span.
function renderContext(context: string) {
  // Split on **word** while keeping the captured segments.
      ...
      {parts.map((part, i) => {
        ...
        if (match) {
          return (
            <span
              key={i}
              className="inline-block mx-1 px-2 py-0.5 bg-white border-b-4 border-rose-400 rounded text-rose-700 font-black uppercase"
            >
              {match[1]}
            </span>
          );
...
function ResultsCard({
  ...
  const idempotencyKey = useMemo(() => newIdempotencyKey('la-vocab'), []);
