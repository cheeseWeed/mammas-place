...
// Language Arts → Grammar drill (Phase L2).
//
// One mixed drill that pulls from three skill threads (parts-of-speech,
// agreement, tense). Each item carries its own `skill` tag so wrong-answer
// analytics can later surface "you keep missing tense."
//
// Three views in one route:
//   1) Config:   pick tier (easy/medium/hard), optional skill filter, round size.
//   2) Playing:  one prompt at a time with choices. On wrong: show the rule
//                + correct answer for ~1.6s, then advance.
//   3) Results:  score, accuracy, MP earned. PendingEarnPrompt for anon kids.
//
// Same MP wiring as Homophones / Math: server-decided cents via
// /api/money/earn, idempotency-keyed, no daily cap.
...
type SkillFilter = 'all' | GrammarSkill;
...
const DEFAULT_CONFIG: Config = { tier: 'easy', skill: 'all', questions: 10 };
const ROUND_SIZES = [5, 10, 15, 20] as const;

type View =
  | { kind: 'config' }
  | { kind: 'playing'; items: GrammarItem[]; config: Config }
...
function GrammarAuthed() {
  const [view, setView] = useState<View>({ kind: 'config' });

  const startRound = useCallback((config: Config) => {
    const skills = config.skill === 'all' ? undefined : [config.skill];
    const items = pickRoundItems({ tier: config.tier, skills }, config.questions);
    if (items.length === 0) {
      // Tier × skill combo with zero items — bail rather than render empty.
      return;
    }
...
const TIERS: { key: GrammarTier; label: string; blurb: string }[] = [
  { key: 'easy', label: 'Easy', blurb: 'nouns, verbs, he runs' },
  { key: 'medium', label: 'Medium', blurb: 'adverbs, went/ate/ran' },
  { key: 'hard', label: 'Hard', blurb: 'neither / had been' },
];

const SKILLS: { key: SkillFilter; label: string; blurb: string }[] = [
  { key: 'all', label: 'All', blurb: 'mix it up' },
  { key: 'parts-of-speech', label: 'Parts of Speech', blurb: 'noun, verb, adj, adv' },
  { key: 'agreement', label: 'Agreement', blurb: 'he runs / they run' },
  { key: 'tense', label: 'Tense', blurb: 'past / present / future' },
];

function ConfigCard({ onStart }: { onStart: (c: Config) => void }) {
  const [tier, setTier] = useState<GrammarTier>(DEFAULT_CONFIG.tier);
  const [skill, setSkill] = useState<SkillFilter>(DEFAULT_CONFIG.skill);
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-rose-200 p-6 md:p-8">
      <div className="text-center mb-6">
        ...
        <h2 className="text-2xl md:text-3xl font-black text-rose-900">Grammar drill</h2>
        <p className="text-sm text-rose-700 mt-1">
          Parts of speech, subject-verb agreement, and verb tense.
        </p>
      </div>
...
const SKILL_LABEL: Record<GrammarSkill, string> = {
...
function PlayCard({
  ...
  // Fill-in-blank items contain `____`; parts-of-speech items don't.
  const hasBlank = current.sentence.includes('____');
    ...
    <div className="bg-white rounded-3xl shadow-xl border-2 border-rose-200 p-6 md:p-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs font-semibold text-rose-700 uppercase tracking-wide">
          {SKILL_LABEL[current.skill]} · {config.tier}
        </span>
      </div>

      <div className="text-center mb-2">
        <div className="text-sm md:text-base font-bold text-rose-700 mb-3">
          {current.prompt}
        </div>
...
// Renders a sentence containing `____` with the blank highlighted.
...
function ResultsCard({
  ...
  const idempotencyKey = useMemo(() => newIdempotencyKey('la-gram'), []);
