...
// Language Arts → Dictionary Skills drill (Phase L5).
//
// Mirrors the Homophones (L1) drill shape so the kid sees one rhythm across
// every Language Arts phase. Three views:
//
//   1) Config:   pick tier (easy/medium/hard) + optional skill focus +
//                round size.
//   2) Playing:  one question at a time. Multiple-meanings items also show
//                the surrounding sentence as a "context" card.
//                On wrong: show the kid-readable rule for ~1.6s.
//   3) Results:  score, MP earned via /api/money/earn, Play Again /
//                Change settings.
//
// MP wiring is server-decided + idempotency-keyed, same as the rest of the
// site (math, spelling, homophones, geography).
...
import {
  ...
  pickRoundItems,
  type DictionaryItem,
  type DictionarySkill,
  type DictionaryTier,
} from '@/lib/languageArts/dictionary';
import {
  isPending,
  newIdempotencyKey,
  submitEarn,
...
type SkillFilter = DictionarySkill | 'mixed';
...
type View =
  | { kind: 'config' }
  | { kind: 'playing'; items: DictionaryItem[]; config: Config }
...
function DictionaryAuthed() {
  ...
  const startRound = useCallback((config: Config) => {
    const items = pickRoundItems(
      {
        tier: config.tier,
        skill: config.skill === 'mixed' ? undefined : config.skill,
      },
      config.questions,
    );
    if (items.length === 0) {
      // Tier/skill combo with zero items shouldn't happen with current
      // dataset, but guard anyway so we don't render an empty drill.
      return;
    }
...
const TIERS: { key: DictionaryTier; label: string; blurb: string }[] = [
  { key: 'easy',   label: 'Easy',   blurb: 'first-letter & basics' },
  { key: 'medium', label: 'Medium', blurb: 'closer comparisons' },
  { key: 'hard',   label: 'Hard',   blurb: 'origins & stress' },
];

function ConfigCard({ onStart }: { onStart: (c: Config) => void }) {
  const [tier, setTier] = useState<DictionaryTier>(DEFAULT_CONFIG.tier);
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-rose-200 p-6 md:p-8">
      <div className="text-center mb-6">
        ...
        <h2 className="text-2xl md:text-3xl font-black text-rose-900">Dictionary Skills</h2>
        <p className="text-sm text-rose-700 mt-1">
          Alphabetical order, guide words, pronunciation, multiple meanings, and more.
        </p>
      </div>
      ...
      <fieldset className="mb-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <SkillButton
            ...
            emoji="🎲"
            on={skill === 'mixed'}
            onClick={() => setSkill('mixed')}
          />
          {DICTIONARY_SKILLS.map((s) => (
            <SkillButton
              key={s.key}
              label={s.label}
              emoji={s.emoji}
...
function SkillButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-xl border-2 px-2 py-3 font-bold text-xs transition-all leading-tight ${
        on
          ? 'border-rose-600 bg-rose-600 text-white shadow-md'
          : 'border-rose-200 bg-rose-50 text-rose-900 hover:border-rose-400'
      }`}
...
function PlayCard({
  ...
  const skillLabel =
    DICTIONARY_SKILLS.find((s) => s.key === current.skill)?.label ?? current.skill;
    ...
    <div className="bg-white rounded-3xl shadow-xl border-2 border-rose-200 p-6 md:p-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs font-semibold text-rose-700 uppercase tracking-wide">
          {skillLabel} · {config.tier}
        </span>
      </div>

      {current.context && (
        <div className="mb-4 rounded-2xl bg-rose-50 border-2 border-rose-200 p-4">
          <div className="text-xs font-bold text-rose-700 uppercase tracking-wide mb-1">
            Sentence
          </div>
          <div className="text-base md:text-lg text-rose-900 italic">
        ...
        {current.choices.map((choice) => {
          ...
          return (
            <button
              ...
              className={`rounded-2xl border-2 px-4 py-4 font-bold text-base text-left transition-all ${cls} disabled:cursor-not-allowed`}
            >
              {choice}
              {isPickedRight && <span className="ml-2">✓</span>}
              {isPickedWrong && <span className="ml-2">✗</span>}
              {isCorrectAnswer && <span className="ml-2 text-sm font-semibold">(correct)</span>}
            </button>
          );
        })}
      </div>

      {reveal !== null && !reveal.correct && current.rule && (
...
function ResultsCard({
  ...
  const idempotencyKey = useMemo(() => newIdempotencyKey('la-dict'), []);
