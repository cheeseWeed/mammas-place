...
// Language Arts → Phonics & Sounds drill (Phase L4).
//
// Three views in one route:
//   1) Config:   pick tier (easy/medium/hard), round size.
//   2) Playing:  one prompt at a time. Modes rotate (find-sound,
//                identify-sound, find-blend, long-vs-short). On wrong:
//                show the rule for ~1.6s then advance.
//   3) Results:  score, accuracy, MP earned, Play Again / New Settings.
//
// MP wiring matches Homophones: server-decided cents via /api/money/earn,
// idempotency-keyed, no daily cap. submitEarn('languageArts', 'drill', ...).
...
type View =
  | { kind: 'config' }
  | { kind: 'playing'; items: PhonicsItem[]; config: Config }
...
const TIERS: { key: PhonicsTier; label: string; blurb: string }[] = [
  { key: 'easy', label: 'Easy', blurb: 'short vowels' },
  { key: 'medium', label: 'Medium', blurb: 'blends & digraphs' },
  { key: 'hard', label: 'Hard', blurb: 'vowel teams' },
];

function ConfigCard({ onStart }: { onStart: (c: Config) => void }) {
  const [tier, setTier] = useState<PhonicsTier>(DEFAULT_CONFIG.tier);
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-rose-200 p-6 md:p-8">
      <div className="text-center mb-6">
        ...
        <h2 className="text-2xl md:text-3xl font-black text-rose-900">Phonics & Sounds</h2>
        <p className="text-sm text-rose-700 mt-1">
          Listen for the sound, then pick the answer.
        </p>
      </div>
...
const SKILL_LABEL: Record<PhonicsItem['skill'], string> = {
  vowels: 'Vowels',
  blends: 'Blends',
  digraphs: 'Digraphs',
...
function PlayCard({
  ...
  // Shuffle the choices per item so the answer doesn't sit in the same slot
  // for an entire round. Memoised by item id so re-renders during reveal
  // don't reshuffle and confuse the kid.
  const choices = useMemo(() => shuffleChoices(current), [current]);
    ...
    <div className="bg-white rounded-3xl shadow-xl border-2 border-rose-200 p-6 md:p-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-pink-700 bg-pink-100 rounded-full px-2.5 py-0.5">
...
function ResultsCard({
  ...
  const idempotencyKey = useMemo(() => newIdempotencyKey('la-pho'), []);
