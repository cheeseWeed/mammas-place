// Phase 2 — Name the State quiz.
//
// Three UI states:
//   1. Picker         — round size + region + hover-toggle.
//   2. In Progress    — QuizEngine renders mid-round.
//   3. Results        — score + missed states + replay.
'use client';
...
export default function NameQuizPage() {
  ...
  function handleComplete(roundResult: { score: number; total: number; misses: string[] }) {
    ...
    const prev = readPhase('name-quiz') ?? { attempts: 0, bestScore: 0, misses: [] };
    ...
    const key = newIdempotencyKey('geo-name-quiz');
    void submitEarn(
      'geography',
      'quiz',
      { correct: roundResult.score, total: roundResult.total, quiz: 'name-quiz' },
      key,
    ).then((res) => {
      if ('error' in res) {
  ...
  // Effective question count = min(picked size, pool size). E.g. picking
  // "20" with region="Northeast" (9 states) gives a 9-question round.
  ...
  return (
    <div className="w-full h-[calc(100vh-142px)] max-sm:h-[calc(100vh-218px)] flex flex-col bg-gradient-to-b from-emerald-50 via-sky-50 to-white relative">
      {/* IN-ROUND top strip: back link + hover toggle, very thin */}
      {inRound && (
      ...
      {inRound && (
        <div className="flex-1 min-h-0">
          <QuizEngine
            ...
            showOppositeLabels={showCapitals}
      ...
      {showOverlay && (
        <>
          ...
          <div className="shrink-0 bg-white/70 backdrop-blur-sm border-b border-emerald-100 px-3 sm:px-4 py-2 flex items-center gap-x-4 flex-wrap">
            ...
            <h2 className="text-base sm:text-lg font-black text-emerald-900">Name the State</h2>
          </div>
      ...
      {result.misses.length > 0 && (
        <div className="bg-emerald-50 rounded-xl p-3 md:p-4 mb-4 border border-emerald-100">
          <h3 className="text-xs font-bold text-emerald-900 mb-2 uppercase tracking-wide">
            States to review ({result.misses.length})
          </h3>
