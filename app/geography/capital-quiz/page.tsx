// Phase 3 — Find the Capital quiz.
//
// Three UI states: Picker → In Progress → Results.
// Mirrors name-quiz: region selector, hover-to-peek toggle, opposite-labels
// anchor toggle (here = show state names as scaffolding).
'use client';

import { useState } from 'react';
import Link from 'next/link';
import statesData from '@/data/states.json';
import { QuizEngine, USMap } from '@/components/geography';
...
type StateRecord = { postal: string; name: string; capital: string; region?: string };
...
function capitalForPostal(postal: string): string | null {
  return STATES.find((s) => s.postal === postal)?.capital ?? null;
}
...
export default function CapitalQuizPage() {
  ...
  function handleComplete(roundResult: { score: number; total: number; misses: string[] }) {
    ...
    const prev = readPhase('capital-quiz') ?? { attempts: 0, bestScore: 0, misses: [] };
    ...
    if (learner) {
      const key = newIdempotencyKey('geo-capital-quiz');
      void submitEarn(
        'geography',
        'quiz',
        { correct: roundResult.score, total: roundResult.total, quiz: 'capital-quiz' },
        key,
      ).then((res) => {
        if ('error' in res) {
    ...
    <div className="w-full h-[calc(100vh-142px)] max-sm:h-[calc(100vh-218px)] flex flex-col bg-gradient-to-b from-emerald-50 via-sky-50 to-white relative">
      {inRound && (
        <div className="shrink-0 bg-white/80 backdrop-blur-sm border-b border-emerald-200 px-3 sm:px-4 py-2 flex items-center gap-x-4 gap-y-1 flex-wrap">
          ...
          <div className="ml-auto flex items-center gap-3 sm:gap-5 flex-wrap">
            <span className="text-xs font-bold uppercase tracking-wide text-gray-500">
              Helpers:
            </span>
            <label className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-800 cursor-pointer hover:text-emerald-700 transition-colors">
              <input
                type="checkbox"
                checked={showStates}
              ...
              />
              Show state names on map
            </label>
            <label className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-800 cursor-pointer hover:text-emerald-700 transition-colors">
              <input
                type="checkbox"
                checked={allowHover}
                onChange={(e) => setAllowHover(e.target.checked)}
                className="w-4 h-4 accent-emerald-600 cursor-pointer"
              />
              Hover shows both
            </label>
      ...
      {inRound && (
        <div className="flex-1 min-h-0">
          <QuizEngine
            ...
            questionCount={effectiveCount as 5 | 10 | 20 | 50}
            mode="capital"
            region={activeRegion}
            allowHoverNames={allowHover}
            showOppositeLabels={showStates}
      ...
      {showOverlay && (
        <>
          <div className="shrink-0 bg-white/70 backdrop-blur-sm border-b border-emerald-100 px-3 sm:px-4 py-2 flex items-center gap-x-4 flex-wrap">
            ...
            <h2 className="text-base sm:text-lg font-black text-emerald-900">Find the Capital</h2>
          </div>

          <div className="flex-1 min-h-0 w-full px-1 sm:px-2 py-1 pointer-events-none opacity-40" aria-hidden>
            <USMap showCapitalStars={true} showCapitalNames={false} showStateLabels={false} />
          </div>
...
const REGIONS: Array<{ value: Region; label: string; desc: string }> = [
  ...
  { value: 'South', label: 'South', desc: '16 states + DC' },
  ...
  return (
    <div className="bg-white/95 backdrop-blur rounded-2xl shadow-2xl border border-emerald-100 p-4 md:p-5">
      <h2 className="text-lg md:text-xl font-black text-emerald-900 text-center mb-3">
        Pick a region, then a round size
      </h2>
...
function ResultsCard({
  ...
  const cheer = perfect
    ? 'Every single capital — nice work.'
    : pct >= 80 ? 'Great round!'
        ...
        <div className="bg-emerald-50 rounded-xl p-3 md:p-4 mb-4 border border-emerald-100">
          ...
          <div className="flex flex-wrap gap-1.5">
            {result.misses.map((postal) => {
              const cap = capitalForPostal(postal);
              const stateName = nameForPostal(postal);
              const label = cap ? `${cap} (${stateName})` : stateName;
              return (
                <span
                  key={postal}
