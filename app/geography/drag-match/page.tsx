// Phase 4 — Drag & Match.
//
// Mirrors the name-quiz page structure: picker → in-round → results.
// Picker controls: region filter + round size. In-round renders DragMatchEngine.
// Results reuses the same card pattern.
'use client';

import { useState } from 'react';
import Link from 'next/link';
import statesData from '@/data/states.json';
import { DragMatchEngine, USMap } from '@/components/geography';
...
export default function DragMatchPage() {
  ...
  function handleComplete(roundResult: { score: number; total: number; misses: string[] }) {
    ...
    const prev = readPhase('drag-match') ?? { attempts: 0, bestScore: 0, misses: [] };
    ...
    if (learner) {
      const key = newIdempotencyKey('geo-drag-match');
      void submitEarn(
        'geography',
        'quiz',
        { correct: roundResult.score, total: roundResult.total, quiz: 'drag-match' },
        key,
      ).then((res) => {
        if ('error' in res) {
  ...
  const effectiveCount = inRound ? Math.min(activeSize!, poolSize(activeRegion)) : null;
  ...
  return (
    <div className="w-full h-[calc(100vh-142px)] max-sm:h-[calc(100vh-218px)] flex flex-col bg-gradient-to-b from-emerald-50 via-sky-50 to-white relative">
      {/* IN-ROUND top strip: back link + region tag */}
      {inRound && (
      ...
      {showOverlay && (
        <>
          <div className="shrink-0 bg-white/70 backdrop-blur-sm border-b border-emerald-100 px-3 sm:px-4 py-2 flex items-center gap-x-4 flex-wrap">
            ...
            <h2 className="text-base sm:text-lg font-black text-emerald-900">Drag &amp; Match</h2>
          </div>
...
function RoundPicker({ onPick }: { onPick: (size: RoundSize, region: Region) => void }) {
  ...
  return (
    <div className="bg-white/95 backdrop-blur rounded-2xl shadow-2xl border border-emerald-100 p-4 md:p-5">
      ...
      <p className="text-center text-xs sm:text-sm text-emerald-700 mb-3">
        Drag each name onto the right state. Or tap a name then tap a state.
      </p>
          ...
          {REGIONS.map((r) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`text-[10px] leading-tight ${active ? 'text-emerald-50/90' : 'text-gray-500'}`}
                >
                  {r.desc}
                </div>
              </button>
            ...
            return (
              ...
              >
                ...
                <div className="text-[10px] text-emerald-50/90 leading-tight">
                  {effective} tile{effective === 1 ? '' : 's'} · {c.sub}
                </div>
              </button>
            );
          })}
    ...
    <div className="bg-white/95 backdrop-blur rounded-2xl shadow-2xl border border-emerald-100 p-5 md:p-6 max-h-[80vh] overflow-y-auto">
      <div className="text-center mb-4">
        ...
        <p className="text-sm text-emerald-700">
          {result.region ? `${result.region} round · ` : ''}
          {cheer}
        </p>
      </div>
