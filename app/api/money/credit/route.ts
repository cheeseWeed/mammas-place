// POST /api/money/credit
// Parent-gated. Body: { user, cents, reason, type? }
// Adds money to a kid's balance.
import { NextRequest, NextResponse } from 'next/server';
import { isValidUser } from '@/lib/drive-progress';
import { credit, TransactionType } from '@/lib/money/balance';
import { isParentAuthenticated } from '@/lib/money/parent';

const ALLOWED_TYPES: TransactionType[] = ['earn', 'gift', 'adjust', 'refund'];
...
export async function POST(req: NextRequest) {
  ...
  let body: { user?: unknown; cents?: unknown; reason?: unknown; type?: unknown };
  try {
    body = await req.json();
  } catch {
  ...
  const type = (typeof body.type === 'string' && ALLOWED_TYPES.includes(body.type as TransactionType))
    ? (body.type as TransactionType)
    : 'earn';

  try {
    const balanceCents = await credit(body.user as string, cents, type, reason);
